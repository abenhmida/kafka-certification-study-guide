# Apache Kafka Certification Mastery — Table of Contents

Welcome to the Kafka Certification Mastery repository. Below is a comprehensive table of contents referencing all chapters and resources in this project.

## Chapters

- [**Chapter 1: Kafka Mental Model & Fundamentals**](kafka-certification-mastery-chapter-1.md)
  - Core concepts: Topics, Partitions, Brokers, Producers, Consumers, and Offsets.
- [**Chapter 2: Kafka Architecture Deep Dive**](kafka-certification-mastery-chapter-2.md)
  - KRaft, Control Plane vs Data Plane, and Broker internals.
- [**Chapter 3: Topics, Partitions, Offsets and Log Internals**](kafka-certification-mastery-chapter-3.md)
  - Detailed look at partition ordering, storage, and retention.
- [**Chapter 4: Producers Deep Dive**](kafka-certification-mastery-chapter-4.md)
  - Idempotence, Transactions, and Performance tuning.
- [**Chapter 5: Consumers and Consumer Groups Deep Dive**](kafka-certification-mastery-chapter-5.md)
  - Group coordination, Rebalancing, and Offset management.
- [**Chapter 6: Kafka Storage, Replication, ISR and Fault Tolerance**](kafka-certification-mastery-chapter-6.md)
  - Durability, ISR, High Watermark, and Failure scenarios.
- [**Chapter 7: Consumer Groups**](Kafka_Certification_Chapter_07_Consumer_Groups.md)
  - Detailed scenarios, Assignment strategies, and Operations.

## Supplementary Resources

- [**Kafka Internal Architecture**](kafka-certification-mastery-internal-architecture.md)
  - A deep dive into internals with Graphviz/DOT diagrams.
- [**Kafka Developer Certification Mock Exam**](kafka_developer_certification_mock_exam.md)
  - Practice questions for CCDAK/CCAAK certification.

## Project Planning

- [**Planning**](planning.md)
  - Project milestones and roadmap.
